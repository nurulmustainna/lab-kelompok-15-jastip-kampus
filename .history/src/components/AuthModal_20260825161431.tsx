  ...
  const handleUnifiedLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (cleanInput === 'kelompok15@gmail.com' || cleanInput === 'kelompok15' || cleanInput === 'admin') {
        ...
        console.log('✅ [DEBUG] Admin login success, setting 500ms timeout');
        
        const adminAccount = adminAcc;
        
        setTimeout(() => {
          console.log('✅ [DEBUG] Admin 500ms timeout fired, calling onSelectStudent');
          try {
            onSelectStudent(adminAccount);
            console.log('✅ [DEBUG] Admin onSelectStudent called');
          } catch (err) {
            console.error('❌ [ERROR] Admin onSelectStudent error:', err);
          }
          
          try {
            onClose();
            console.log('✅ [DEBUG] Admin onClose called');
          } catch (err) {
            console.error('❌ [ERROR] Admin onClose error:', err);
          }
        }, 500);
        return;
      }
      ...
      if (cleanInput === 'jasatitip@gmail.com' || cleanInput === 'jasatitip' || cleanInput === 'jastiper') {
        ...
        console.log('✅ [DEBUG] Jastiper login success, setting 500ms timeout');
        
        const jasterAccount = jastiperAcc;
        
        setTimeout(() => {
          console.log('✅ [DEBUG] Jastiper 500ms timeout fired, calling onSelectStudent');
          try {
            onSelectStudent(jasterAccount);
            console.log('✅ [DEBUG] Jastiper onSelectStudent called');
          } catch (err) {
            console.error('❌ [ERROR] Jastiper onSelectStudent error:', err);
          }
          
          try {
            onClose();
            console.log('✅ [DEBUG] Jastiper onClose called');
          } catch (err) {
            console.error('❌ [ERROR] Jastiper onClose error:', err);
          }
        }, 500);
        return;
      }
      ...
      if (foundMock) {
        ...
        console.log('✅ [DEBUG] Login success for mahasiswa, setting 500ms timeout before calling onSelectStudent');
        
        // Store the account in a variable to ensure it's available in the callback
        const loginAccount = foundMock;
        
        setTimeout(() => {
          console.log('✅ [DEBUG] 500ms timeout fired, calling onSelectStudent with account:', loginAccount.name);
          try {
            onSelectStudent(loginAccount);
            console.log('✅ [DEBUG] onSelectStudent called successfully');
          } catch (err) {
            console.error('❌ [ERROR] onSelectStudent threw error:', err);
          }
          
          try {
            console.log('✅ [DEBUG] Calling onClose to close modal');
            onClose();
            console.log('✅ [DEBUG] onClose called successfully');
          } catch (err) {
            console.error('❌ [ERROR] onClose threw error:', err);
          }
        }, 500);
        return;
      }
