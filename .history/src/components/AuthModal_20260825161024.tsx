  ...
  const handleUnifiedLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (foundMock) {
        ...
        setTimeout(() => {
          console.log('✅ [DEBUG] 500ms timeout fired, calling onSelectStudent with account:', foundMock.name);
          onSelectStudent(foundMock);
          console.log('✅ [DEBUG] onSelectStudent called, now calling onClose');
          onClose();
          console.log('✅ [DEBUG] onClose called, modal should close now');
        }, 500);
        return;
      }
